...
# by Dr Ye Wang, Department EEE, The University of Melbourne
...
def KTMPC_optimiser(z0,zt,zrange,urange,A,B,Q,R,S,N):
    ...
    umin = np.array([0.05, -urange[1]])

    # Define constraints
    cons = [z[:,0]==z0,z[:,-1]==zs]
    cons += [zs == A @ zs + B @ us,
             us >= usmin, us <= urange]
             # zs[0:3] >= -zrange, zs[0:3] <= zrange]
             # us >= -urange, us <= urange]
...
def KTMPC_simulation2(model_file,yt,init_state,xrange,urange,q,r,s,N,simtime,changedt,model_flag):
    ...
    for i in range(simtime):
        ...
        if flag not in ["infeasible","unbounded"]:
            ...
            # print(np.linalg.norm(xk[0:1] - ysk[0:1]))
            # if np.linalg.norm(xk[0]-yt[j,0]) < 0.1 and np.linalg.norm(xk[1]-yt[j,1]) < 0.1:
            if np.linalg.norm(xk[0:2] - yt[j,0:2]) <= 0.05: #or np.linalg.norm(xk[0:2] - xpk[0:2]) <= 0.001:
            # if i>0 and i%changedt == 0:
                j += 1
                ypr = yt[j,0:2]
...
def KTMPC_simulation_plot(x,z,u,yt,zr,yr,ys,us,urange,simtime,Ts):
    ...
    axs[0].plot(tseries, us[0,:], color='red', linestyle='dashed', label='steady input')
    ...
    axs[1].plot(tseries, us[1,:], color='red', linestyle='dashed', label='steady input')
...
def KTMPC_simulation_plot_x(x,z,u,yt,zr,yr,ys,us,urange,simtime,Ts):
    ...
    axs[0].plot(tseries1, x[0,:], color='black',label='KTMPC')
    axs[0].set_ylabel(r'$x$')
    axs[0].legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=3, mode="expand")
...
def KTMPC_simulation_plot_u(x,z,u,yt,zr,yr,ys,us,urange,simtime,Ts):
